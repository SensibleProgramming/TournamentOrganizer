...
description: Implement all unblocked Ready items in parallel, each in its own git worktree.
---
...
> Follow all steps in the `implement-next` skill starting from Step 3 (branch already determined: `feature/SHORT_NAME`). Use its established agent dispatches (`build-runner`, `zone-auditor`, `e2e-runner`, `github-actor`) for build/zone/e2e/PR work rather than re-deriving those procedures.
