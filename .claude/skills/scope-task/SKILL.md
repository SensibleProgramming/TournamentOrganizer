---
name: scope-task
description: Analyze a GitHub issue, estimate story points, recommend the right Claude model, and optionally launch implementation.
---
...
Find the item whose `content.number` matches. Extract `ITEM_ID`, `ISSUE_NUMBER`, `ISSUE_TITLE`.
...
Read the prompt file and score the following signals (this is a judgement call — keep it inline, do not delegate):
...
```
pwsh -File .claude/scripts/gh-board.ps1 -Action SetPoints -Item $ITEM_ID -Value <estimated-points>
```
