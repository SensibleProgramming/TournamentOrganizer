...
Dispatch one `api-fuzzer` agent (sonnet) per controller group. Each agent receives:
- The list of `{path, method, parameters, requestBody}` for its controller group
- The payload battery below
...
For each remaining finding, dispatch one `github-actor` agent (haiku) in parallel. Each agent:
1. Writes the finding's full body (summary, location, reproduction, suggested fix) to a temp markdown file
2. Runs: `.claude/scripts/gh-issue-file.ps1 -Severity <SEVERITY> -Prefix "[Fuzz]" -Title "<title>" -BodyFile <path>`

That single script call creates the issue, adds it to the board, and sets Status=Backlog + Priority — replacing what used to be a ~80-line hand-written per-finding template (gh issue create with a heredoc body, then three separate `gh project item-edit` calls with hardcoded field/option IDs).
