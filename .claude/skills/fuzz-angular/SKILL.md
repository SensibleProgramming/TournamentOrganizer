...
1. Writes the finding's full body (summary, location, reproduction, suggested fix, OWASP reference) to a temp markdown file
2. Runs: `.claude/scripts/gh-issue-file.ps1 -Severity <SEVERITY> -Prefix "[FuzzUI]" -Title "<title>" -BodyFile <path>`

That single script call creates the issue, adds it to the board, and sets Status=Backlog + Priority.
