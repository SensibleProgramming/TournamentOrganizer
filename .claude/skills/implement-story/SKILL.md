...
description: Pick the first Ready item from the GitHub project board and implement it. Git commits, pushes, and PR creation are handled by the user - stop after implementation passes all checks.
---
...
This skill follows **all steps in the `implement-next` skill** with the following two differences:
...
Follow the `implement-next` skill exactly: issue loading, prompt file generation/approval, dependency check, board "In Progress" update (via `github-actor`), branch creation, TDD implementation, build/zone/E2E verification (via `build-runner`/`zone-auditor`/`e2e-runner`), and prompt file move.
